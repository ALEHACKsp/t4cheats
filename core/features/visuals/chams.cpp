...
void override_material(bool ignorez, bool flat, const color& color) {
	//finding materials
	const auto material_shaded = interfaces::material_system->find_material("debug/debugambientcube", TEXTURE_GROUP_MODEL);
	const auto material_flat = interfaces::material_system->find_material("debug/debugdrawflat", TEXTURE_GROUP_MODEL);

	material = flat ? material_flat : material_shaded; //use flat / shaded material

	material->color_modulate(color.r / 255.f, color.g / 255.f, color.b / 255.f); //change color of the material
	material->alpha_modulate(color.a / 255.f); //change opacity of the material
