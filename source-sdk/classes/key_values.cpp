...
key_values* key_values::find_key(const char* key_name, bool create) {
    static const auto key_values_find_key = relative_to_absolute<key_values*(__thiscall*)(key_values*, const char*, bool)>(reinterpret_cast<int*>("client.dll", "E8 ? ? ? ? F7 45", 1));
    return key_values_find_key(this, key_name, create);
}

key_values* key_values::from_string(const char* name, const char* value) {
    static const std::uintptr_t key_values_from_string = relative_to_absolute<std::uintptr_t>(reinterpret_cast<int*>(utilities::pattern_scan("client.dll", "E8 ? ? ? ? 83 C4 04 89 45 D8") + 1));
...
void key_values::set_string(const char* key_name, const char* value) {
    static const auto key_values_set_string = relative_to_absolute<void(__thiscall*)(key_values*, const char*)>(reinterpret_cast<int*>("client.dll", "E8 ? ? ? ? 89 77 38", 1));
